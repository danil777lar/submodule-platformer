...
public class LevelEditorToolRect : LevelEditorTool
{
    ...
    private List<Vector2Int> ComputePoints(LevelEditorToolArgs args, Vector2Int position)
    {
        ...
        for (int i = Mathf.Min(_pointerDownPosition.x, position.x); i <= Mathf.Max(_pointerDownPosition.x, position.x); i++)
        {
            Vector2Int startYPoint = new Vector2Int(i, _pointerDownPosition.y);
            if (!points.Contains(startYPoint))
            {
                points.Add(startYPoint);
            }
            Vector2Int endYPoint = new Vector2Int(i, position.y);
            if (!points.Contains(endYPoint))
            {
                points.Add(endYPoint);
            }
        }
        for (int i = Mathf.Min(_pointerDownPosition.y, position.y); i <= Mathf.Max(_pointerDownPosition.y, position.y); i++)
        {
            Vector2Int startXPoint = new Vector2Int(_pointerDownPosition.x, i);
            if (!points.Contains(startXPoint))
            {
                points.Add(startXPoint);
            }
            Vector2Int endXPoint = new Vector2Int(position.x, i);
            if (!points.Contains(endXPoint))
            {
                points.Add(endXPoint);
            }
        }
